...
宿主侧用 C# 实现静态或可实例化 Native Type 时，阅读 MCP 资源 `aurora://docs/host-integration` 与 `aurora://schema/host-api` 中的 `[NativeType]` / `[Export]` / `INativeTypedDocument` 说明；不要猜测 Bonding 回调签名。应用类型通过 `AddNativeType<T>()`、`WithNativeTypes(...)` 或 `AddNativeTypes(...)` 加入引擎 catalog；`Math` / `JSON` / `TDoc` / `console` / `Conv8` / `Env` / `HotPatch` 是基础设施 Type，无需列出。脚本 `tdoc Vec2 { ... }` / `tdoc Vec2 [ ... ]` / `tdoc Flag false` 以及宿主 TDoc 序列化要求 NativeType 实现 `INativeTypedDocument`。脚本侧在 `@global()` 文件中手写 `declare type`，不要使用 `declare class` 或 `declare module`。`native func ... void` 与 packed array 可空参数见 `aurora://docs/language`、`aurora://docs/performance` 与 `aurora://schema/features`。语法索引见 `aurora://schema/features` 与 `aurora://schema/ebnf`。
