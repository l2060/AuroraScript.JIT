...
namespace AuroraScript.Runtime.Types.TypeConstruct
{
    internal sealed class PathConstructor : ScriptType
    {
        ...
        {
            var root = ScriptPathValue.GetPathString(args, 0);
            ScriptDatum.WriteAsString(ref result, PathText.Build(root, args, 1));
        }

        internal static void NORMALIZE(ScriptContext ctx, ScriptObject thisObject, Span<ScriptDatum> args, ref ScriptDatum result)
        {
            ScriptDatum.WriteAsString(ref result, PathText.Normalize(ScriptPathValue.GetPathString(args, 0)));
        }
        ...
        {
            ScriptDatum.WriteAsString(ref result, PathText.GetDirectoryName(ScriptPathValue.GetPathString(args, 0)));
        }

        internal static void FILE_NAME(ScriptContext ctx, ScriptObject thisObject, Span<ScriptDatum> args, ref ScriptDatum result)
        {
            ScriptDatum.WriteAsString(ref result, PathText.GetFileName(ScriptPathValue.GetPathString(args, 0)));
        }

        internal static void PROTOCOL(ScriptContext ctx, ScriptObject thisObject, Span<ScriptDatum> args, ref ScriptDatum result)
        {
            ScriptDatum.WriteAsString(ref result, PathText.GetProtocol(ScriptPathValue.GetPathString(args, 0)));
        }
        ...
        internal static void ENSURE_EXTENSION(ScriptContext ctx, ScriptObject thisObject, Span<ScriptDatum> args, ref ScriptDatum result)
        {
            ...
            ScriptDatum.WriteAsString(ref result, PathText.EnsureExtension(path, extension));
        }

        internal static void IS_ROOTED(ScriptContext ctx, ScriptObject thisObject, Span<ScriptDatum> args, ref ScriptDatum result)
        {
            ScriptDatum.WriteAsBoolean(ref result, PathText.IsRooted(ScriptPathValue.GetPathString(args, 0)));
        }
        ...
        internal static void IS_UNDER_ROOT(ScriptContext ctx, ScriptObject thisObject, Span<ScriptDatum> args, ref ScriptDatum result)
        {
            ...
            ScriptDatum.WriteAsBoolean(ref result, PathText.IsUnderRoot(root, path));
        }
        ...
        internal static void CURRENT_DIRECTORY(ScriptContext ctx, ScriptObject thisObject, Span<ScriptDatum> args, ref ScriptDatum result)
        {
            ...
            ScriptDatum.WriteAsString(ref result, PathText.GetDirectoryName(fullPath));
        }
    }
}
