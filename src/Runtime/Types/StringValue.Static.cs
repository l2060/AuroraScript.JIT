...
namespace AuroraScript.Runtime.Types
{
    public sealed partial class StringValue
    {
        ...
        public static int CompareCore(string left, string right)
            => left.Length > 0 && right.Length > 0 ? left[0].CompareTo(right[0]) : 1;

        private static void CREATE(ScriptContext ctx, ScriptObject self, Span<ScriptDatum> args, ref ScriptDatum result)
            => ScriptDatum.WriteAsString(ref result, args.TryGetString(0, out var value) ? value : string.Empty);

        private static void FROMCHARCODE(ScriptContext ctx, ScriptObject self, Span<ScriptDatum> args, ref ScriptDatum result)
            => ScriptDatum.WriteAsString(ref result,
                args.TryGetInteger(0, out var code) ? FromCharCodeCore(unchecked((int)code)) : string.Empty);

        private static void COMPARE(ScriptContext ctx, ScriptObject self, Span<ScriptDatum> args, ref ScriptDatum result)
            => ScriptDatum.WriteAsNumber(ref result,
                args.TryGetString(0, out var left) && args.TryGetString(1, out var right) ? CompareCore(left, right) : 1);
    }
}
