...
- Use `declare type` for `[NativeType]` globals. Static-only Types omit `constructor` and mark members `static`. Do not write `declare class`, `declare module`, or a structural `type` for host Types. Member signatures are editor-only and do not drive compiler inference.
