...
- Extension packages are opt-in host capabilities. `EngineOptions.Default.Packages` is empty; the host must add `NativePackages.FileSystem` and/or `NativePackages.HttpClient` with `WithPackages` before constructing the engine. Do not register those types with `WithNativeTypes`.
...
- Array: writable `length` (shrinking truncates; growing appends `null` slots), `push`, `pop`, `sort`, `join`, `slice`, `reverse`, `unshift`, `shift`, `concat`, `find`, `findIndex`, `findLast`, `findLastIndex`, `map`, `filter`, `some`, `every`, `flat`, `reduce`, `indexOf`, `lastIndexOf`, `has`
